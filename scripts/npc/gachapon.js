  ...
  if (mode < 0) cm.dispose();
    ...
    if (status == 0 && mode == 1) {
      ...
      if (
        ...
        cm.sendOk(
          "请确认在 #r装备, 消耗, 设置, #k和 #r其他#k 栏中至少有一个空格."
        );
      }
      cm.dispose();
    } else if (status == 1) {
      if (selection == 0) {
        cm.sendNext(
          "通过扭蛋获取稀有卷轴, 装备, 椅子, 技能书, 以及其他绝赞物品! 你需要做的仅仅是使用一张 #b扭蛋券#k 来成为抽选随机物品的欧皇."
        );
      } else {
        cm.sendNext(
          "扭蛋券可以在 #r现金商城#k 中通过点券或抵用券购买. 点击右下角中红色商店按钮即可进入 #r现金商城#k."
        );
      }
    } else if (status == 2) {
      cm.sendNextPrev(
